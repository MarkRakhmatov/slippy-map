...
TEST_CASE( "iterate unique tiles", "[MapModel]" ) {
    ...
    bounds.iterateUniqueTiles([&tiles](const geo::Tile& t) {
        tiles.push_back(t);
    });
...
TEST_CASE( "iterate all tiles", "[MapModel]" ) {
    ...
    REQUIRE(bounds.ysize == 4);
    std::vector<geo::Tile> tiles;
    bounds.iterateAllTiles([&tiles](const geo::Tile& t) {
        tiles.push_back(t);
    });
    REQUIRE(tiles[4] == geo::Tile{-1, 4, 1});
    REQUIRE(tiles == std::vector<geo::Tile>{
...
TEST_CASE( "bounds move left", "[Bounds]" ) {
    ...
    bounds1.iterateAllTiles([&tiles1](const geo::Tile& t) {
        tiles1.push_back(t);
    });
    ...
    bounds2.iterateAllTiles([&tiles2](const geo::Tile& t) {
        tiles2.push_back(t);
    });
    REQUIRE( tiles2[0] == geo::Tile{7, 1, 3});
    REQUIRE(tiles2 == std::vector<geo::Tile>{
    ...
    bounds1.iterateUniqueTiles([&uniqueTiles1](const geo::Tile& t) {
        uniqueTiles1.push_back(t);
    });
    REQUIRE(bounds1.getMinMaxX() == std::tuple<int32_t, int32_t>(0, 5));
    auto [miny, maxy] = bounds1.getMinMaxY();
    REQUIRE(miny == 1);
    REQUIRE(maxy == 5);
    REQUIRE(uniqueTiles1 == std::vector<geo::Tile>{
        {0, 1, 3}, {0, 2, 3}, {0, 3, 3}, {0, 4, 3}, {0, 5, 3},
        {1, 1, 3}, {1, 2, 3}, {1, 3, 3}, {1, 4, 3}, {1, 5, 3},
        {2, 1, 3}, {2, 2, 3}, {2, 3, 3}, {2, 4, 3}, {2, 5, 3},
        {3, 1, 3}, {3, 2, 3}, {3, 3, 3}, {3, 4, 3}, {3, 5, 3},
    ...
    bounds2.iterateUniqueTiles([&uniqueTiles2](const geo::Tile& t) {
